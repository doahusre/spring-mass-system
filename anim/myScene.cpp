...
void MakeScene(void)
{
	...
	*/
	...
	success = GlobalResourceManager::use()->addSystem( ps, true );

	assert(success);

	SpringSimulator* springSimulator = new SpringSimulator("partSim");

	success = GlobalResourceManager::use()->addSimulator(springSimulator);
